...
def main(args):
    ...
    sampler = ts.SamplerFactory.get_sampler("SinBaseline", tasks, None, None, capacity=401)
    # sampler = ts.SamplerFactory.get_sampler("Sin2", tasks, None, None, capacity=401)

    config = mf.ModelFactory.get_model(args.model_type, "Sin", in_channels=11, num_actions=30, width=args.width)
            ...
            if step%500 == 0:
                if args.dataset == "Sin":
                    if args.model_type=="old":
                        torch.save(maml, my_experiment.path + "baseline_pretraining_regression_old.net")
                    elif args.model_type=="linear":
                        torch.save(maml, my_experiment.path + "baseline_pretraining_regression_linear.net")
                    elif args.model_type=="nonlinear":
                        torch.save(maml, my_experiment.path + "baseline_pretraining_regression_nonlinear.net")
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--model_type', help='Model type for sine task', default="na")
    ...
    argparser.add_argument('--dataset', help='Name of experiment', default="Sin")
