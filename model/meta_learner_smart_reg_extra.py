...
class MetaLearingClassification(nn.Module):
    ...
    def forward(self, x_spt, y_spt, x_qry, y_qry,old_accs,old_meta_losses,args):
        """
        :param x_traj:   Input data of sampled trajectory
        :param y_traj:   Ground truth of the sampled trajectory
        :param x_rand:   Input data of the random batch of data
        ...
        # Save old parameters
        torch.save(self, 'smart_'+str(args.smart)+'_mini_'+str(args.use_mini)+'_'+'entire_model.pth')

        # Set up variables
        meta_losses = [0.0 for _ in range(x_traj.shape[0])]  # losses_q[i] is the loss on step i
        accuracy_meta_set = [0.0 for _ in range(x_traj.shape[0])]
        fast_weights = self.inner_update(x_traj[0], None, y_traj[0], True)
        ...
        for k in range(1, x_traj.shape[0]-1):
            # Doing inner updates using fast weights
            fast_weights = self.inner_update(x_traj[k], fast_weights, y_traj[k], True)
        ...
        # If new parameters are not better, don't update at outer loop
        if meta_losses[-1] > old_meta_losses[-1]:
            self = torch.load('smart_'+str(args.smart)+'_mini_'+str(args.use_mini)+'_'+'entire_model.pth').to(device)
...
class MetaLearnerRegression(nn.Module):
    ...
    def forward(self, x_spt, y_spt, x_qry, y_qry,old_losses,args):
        ...
        # Save old parameters
        torch.save(self, 'regression_smart_'+str(args.smart)+'_mini_'+str(args.use_mini)+'_'+'entire_model.pth')
